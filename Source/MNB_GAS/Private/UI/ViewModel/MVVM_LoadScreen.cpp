...
{
	// 开始游戏时，根据对应slot，准备加载数据
	AAuraGameModeBase* AuraGameMode = Cast<AAuraGameModeBase>(UGameplayStatics::GetGameMode(this));
	UAuraGameInstance* AuraGameInstacne = Cast<UAuraGameInstance>(AuraGameMode->GetGameInstance());



	AuraGameInstacne->PlayerStartTag = SelectedSlot->PlayerStartTag;
	AuraGameInstacne->LoadSlotName = SelectedSlot->GetLoadSlotName();;
	AuraGameInstacne->LoadSlotIndex = SelectedSlot->SlotIndex;
	if (IsValid(SelectedSlot))
	{
